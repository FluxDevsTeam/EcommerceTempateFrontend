...
const fetchCategoryProducts = async (categoryId: number, page = 1): Promise<ApiResponse> => {
  const response = await fetch(
    `https://api.fluxdevs.com/api/v1/product/item/?is_available=true&category=${categoryId}&page_size=16&page=${page}`
  );
...
const fetchCategoryDetails = async (categoryId: number): Promise<Category> => {
  const response = await fetch(
    `https://api.fluxdevs.com/api/v1/product/category/${categoryId}/`
  );
