...
const ProductPrioritization: React.FC = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://api.fluxdevs.com/api/v1/product/item/?page_size=50`,
        { headers }
      );
  ...
  const handleSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        const response = await fetch(
          `https://api.fluxdevs.com/api/v1/product/item/admin-search/?search=${encodeURIComponent(query)}`,
          { headers }
        );
  ...
  const fetchAllProducts = async () => {
    ...
    let nextPage: string | null = `https://api.fluxdevs.com/api/v1/product/item/?page_size=100`;
    ...
    try {
      ...
      for (const product of allProducts) {
        if (!updatedProducts.some(p => p.id === product.id)) {
          await fetch(
            `https://api.fluxdevs.com/api/v1/product/item/${product.id}/`,
            {
              method: 'PATCH',
              headers,
              body: JSON.stringify({
  ...
  const removeProduct = async (productId: string) => {
    ...
    try {
      ...
      const flagField = activeTab === 'top_selling' ? 'top_selling_items' : 'latest_item';
      const response = await fetch(
        `https://api.fluxdevs.com/api/v1/product/item/${productId}/`,
        {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
  ...
  const addProductFromSearch = async (result: SearchResult) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.fluxdevs.com/api/v1/product/item/${result.id}/`,
        {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
